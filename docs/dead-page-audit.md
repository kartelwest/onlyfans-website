...
| `window.confirm` / `window.prompt` elsewhere | Same suppression risk on every other destructive action: representative deletion (a typed-phrase `prompt`), owner account deletion, earnings-report deletion, ledger-entry deletion, the expenses toggle, and note archiving | **Fixed** — one shared `components/ui/ConfirmDialog` now covers all of them, with a typed phrase where the action is irreversible, a loading state, Escape/backdrop to cancel, and the record shown in the dialog. The only remaining calls are inside components nothing imports |
...
| Missing foreign key | `models.representative_id` had **no FK** in production (only `profile_id` and `created_by` did). PostgREST could not resolve `profiles!representative_id` embeds — PGRST200, whole query failed — and deleting a representative left her models pointing at an account that no longer existed | **Fixed** in 20260803040000: the constraint is on with `ON DELETE SET NULL`, plus a partial index on the column every rep-scoped screen filters by. Zero dangling rows, so it validated without touching data. The code keeps clearing assignments explicitly as well — belt and braces |
