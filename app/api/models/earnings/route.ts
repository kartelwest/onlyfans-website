...
export async function GET(request: NextRequest) {
  try {
    ...
    const reports = await Promise.all(
      (data ?? []).map(async (report) => {
        ...
        if (report.image_path) {
          ...
          if (signed.error) {
            console.error(
              "Erro ao gerar URL assinada do relatório:",
              signed.error,
            );
          } else {
    ...
    console.error(
      "Erro inesperado ao carregar os ganhos:",
      error,
    );
    ...
    return NextResponse.json(
      {
        error:
          "Erro interno ao carregar os ganhos.",
      },
      { status: 500 },
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error(
      "Erro inesperado ao registrar os ganhos:",
      error,
    );
    ...
    return NextResponse.json(
      {
        error:
          "Erro interno ao registrar os ganhos.",
      },
      { status: 500 },
    );
  }
